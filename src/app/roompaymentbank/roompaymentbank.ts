...
export class RoompaymentbankPage implements OnInit{
    ...
    if(!this.activityService.objPaymentMytrip){
      ...
      this.timestamp = Date.now();
        if (se.booking.CEmail) {
          ...
          this.gf.RequestApi('POST', strUrl, headers, body, 'roompaymentbank', 'next').then((data)=>{
            if (data) {
              if (data.error == 0) {
                var code=data.code;
  ...
  openWebpage(url: string) {
    ...
    browser.on('exit').subscribe(() => {
      ...
      let headers =
          ...
          this.gf.RequestApi('GET', strUrl, headers, {}, 'roompaymentbank', 'openWebpage').then((data)=>{
        if (data) {
          var rs = data;
  ...
  nextpayment(){
    this.navCtrl.navigateBack(['mytripaymentselect']);
  }
  nextloadbill()
  {
    this.navCtrl.navigateBack(['mytriloadbill']);
  }
}
