...
export class RegisterverifyPage implements OnInit {
  ...
  confirm() {
    ...
    if (this.num1 && this.num2 && this.num3 && this.num4 && this.num6) {
      ...
      let body =
      {
        PhoneNumber: this.phone, Token: se.obj
      };
      let urlPath = C.urls.baseUrl.urlMobile +'/api/account/OTPRegisterSMS';
      let headers = { 
        'postman-token': 'aac9e726-944b-2180-2416-63d2b1031a7a',
          'cache-control': 'no-cache',
          'content-type': 'application/json'
      };
      this.gf.RequestApi('POST', urlPath, headers, body, 'registerverify', 'confirm').then((data)=>{
  ...
  refreshToken() {
    ...
    se.storage.get('auth_token').then(auth_token => {
      if (auth_token) {
        ...
        this.gf.RequestApi('GET', urlPath, headers, {}, 'registerverify', 'refreshToken').then((data)=>{
  ...
  sendOTP() {
    var se = this;
      ...
      this.gf.RequestApi('POST', urlPath, headers, {PhoneNumber: this.phone}, 'registerverify', 'sendOTP').then((data)=>{
